treeJSON = d3.json("keyword.json", function (error, treeData) {
    ...
    function click(d) {
        ...
        if (!d.children && !d._children && !d.name.includes("MISC")) {
            let folder = './static/' + d.name.substring(0, d.name.indexOf("_")) + '/'
            for (var i = 0; i < 12; i++) {
                images.push(folder + i + '.jpg')
            }
            displayImages(images, 12)
        }
    ...
    function update(source) {
        ...
        nodeEnter.append("text")
            ...
            .text(function (d) {
                if (!d.children && !d._children && !d.name.includes('MISC')) {
                    return d.name.split('"')[1] + ' (' + d.name.split('_')[1] + ')';
                }
                else if (!d.children && !d._children && d.name.includes('MISC'))
        ...
        node.select('text')
            ...
            .text(function (d) {
                if (!d.children && !d._children && !d.name.includes('MISC')) {
                    ...
                    return d.name.substring(0, d.name.lastIndexOf('_')) + ' (' + d.name.substring(d.name.lastIndexOf('_') + 1) + ')';
            });
